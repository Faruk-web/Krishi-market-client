...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->payment_option != null) {
            ...
            $point_order = Order::findOrFail($request->session()->get('order_id'));
            
                    $amounttotal = $request->intotal;
            ...
            if ($request->session()->get('order_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    return $paytm->index();
                     ...
                     $user = Auth::user()->id;
                     $find=ClubPoint::where('user_id',$user)->first();
                    if($find){
                         $cpoint=ClubPoint::find($find->id);
                        $total= $amounttotal;
                    $point = DB::table('pointconfigs')->first();
                    $points=$amounttotal/$point->tk;
                    $points=intval(round($points));
                    $cpoint->points +=$points;
                    $cpoint->save();
                    }else{
                    $cpoint = new ClubPoint;
                    $cpoint->user_id=$user;
                    $total= $amounttotal;
                    $point = DB::table('pointconfigs')->first();
                    $points=$amounttotal/$point->tk;
                    ...
                    $user = Auth::user();
                    $order = Order::findOrFail($request->session()->get('order_id'));
                    if ($user->balance >= $request->intotal) {
                        $user->balance -= $request->intotal;
                        $user->save();
