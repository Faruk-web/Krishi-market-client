<div class="card border-0 shadow-sm rounded">
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th class="product-total text-right">{{translate('Discount Price')}}</th>
                </tr>
            </thead>
            <tbody>
                @php
    ...
    $discounts=\App\Models\Discountconfig::where('product_id',$pid)->get();
   $price=$cartItem['price']*$cartItem['quantity'];
    @endphp
                    @php
                        ...
                        if($discount->start_qty  <= $cartItem['quantity']  && $discount->end_qty >= $cartItem['quantity']){
                        if($discount->type == "Percentage"){
                      $price=  $cartItem['price']*$cartItem['quantity'];
                       $discountt=($discount->discount* $price)/100;
                        $subtotall += $discountt;
                        }
                        if($discount->type == "Flat"){
                      $price=  $cartItem['price']*$cartItem['quantity'];
                        $subtotall += $discount->discount;
                        }
                        }
                    ...
                    <tr class="cart_item">
                        ...
                        <td class="product-total text-right">
                            ...
                            @endphp
                            <span class="pl-4">{{ single_price($price -$discount )  }}</span>
                        </td>
                         @elseif($discount->type == "Flat")
                        <td class="product-total text-right">
                             <span class="pl-4">{{ single_price($price -$discount->discount )  }}</span>
                        </td>
                        ...
                        @else
                        <td class="product-total text-right">
                            <span class="pl-4">{{ single_price(0)  }}</span>
                        </td>
                        @endif
        ...
        <table class="table">
            ...
            <tfoot>
                <tr class="cart-subtotal">
                    ...
                    <td class="text-right">
                        <span class="fw-600">{{ single_price($subtotal-$subtotall) }}</span>
                    </td>
                </tr>
                ...
                <tr class="cart-total">
                    ...
                    <td class="text-right">
                        <strong><span>{{ single_price($total - $subtotall) }}</span></strong>
                        <input type="hidden" name="intotal" value={{$total - $subtotall}}>
                    </td>
                </tr>
            </tfoot>
        </table>
